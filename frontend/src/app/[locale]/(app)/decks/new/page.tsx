...
export default function NewDeckPage() {
  ...
  const createDeckMutation = useMutation({
    ...
    onError: (error: any) => {
      const errorMessage = error.response?.data?.message || t('deckCreatedError');
  ...
  const addCard = () => {
    setValue('cards', [...cards, { front_text: '', back_text: '', color_tag: '' }]);
    setIsDirty(true);
  };
  ...
  const updateCard = (index: number, field: 'front_text' | 'back_text' | 'color_tag', value: string) => {
    ...
    <div className="container mx-auto max-w-4xl p-6 space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={handleCancel} disabled={createDeckMutation.isPending}>
            <ArrowLeft className="mr-2 h-4 w-4" />
            {t('cancel')}
          </Button>
          <Button onClick={handleSubmit(onSubmit)} disabled={createDeckMutation.isPending}>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => { register('name').onChange(e); setIsDirty(true); }}
              />
              {errors.name && (
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(e) => { register('description').onChange(e); setIsDirty(true); }}
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() => { setValue('visibility', 'private'); setIsDirty(true); }}
                  className="flex-1"
                >
                  {t('private')}
                </Button>
                <Button
                  ...
                  onClick={() => { setValue('visibility', 'public'); setIsDirty(true); }}
                  className="flex-1"
                >
                  {t('public')}
                </Button>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">{t('shuffleDesc')}</p>
              </div>
              <Switch
                checked={testShuffle}
                onCheckedChange={(checked) => { setValue('test_shuffle', checked); setIsDirty(true); }}
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">{t('sequentialDesc')}</p>
              </div>
              <Switch
            ...
            {errors.test_shuffle && (
              <p className="text-sm text-destructive">{errors.test_shuffle.message}</p>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              ...
              <Switch
                ...
                onCheckedChange={(checked) => { setValue('study_spaced_repetition', checked); setIsDirty(true); }}
              />
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            {cards.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                <p>{t('noCards')}</p>
                <Button type="button" onClick={addCard} variant="outline" className="mt-4">
                  <Plus className="mr-2 h-4 w-4" />
                  {t('addFirstCard')}
                </Button>
              </div>
                ...
                <Card key={index}>
                  <CardContent className="pt-6 space-y-4">
                    <div className="flex items-start justify-between">
                      <h4 className="font-medium">{t('cardNumber', { number: index + 1 })}</h4>
                      <Button
                        type="button"
                        variant="ghost"
                        size="sm"
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {COLOR_OPTIONS.map((color) => (
                          <button
                            key={color.value}
                            type="button"
                            onClick={() => updateCard(index, 'color_tag', color.value)}
