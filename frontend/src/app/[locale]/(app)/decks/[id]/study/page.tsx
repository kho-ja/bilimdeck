...
export default function DeckStudyPage() {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {t("sessionCompleteMessage", { count: reviewedCount })}
          </CardDescription>
        </CardHeader>
  ...
  const progressPercent = total
    ? Math.round((progressCurrent / total) * 100)
    : 0;
  const colorTag = currentCard?.colorTag;
  const colorClass = colorTag
    ? COLOR_TAG_STYLES[colorTag] || "border-border text-muted-foreground"
    : "border-border";

  return (
    <div className="space-y-6">
      <section className="rounded-3xl border bg-gradient-to-br from-slate-50 via-background to-emerald-50 p-6 shadow-sm dark:border-white/10 dark:from-slate-900/40 dark:via-background dark:to-emerald-900/20">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {isFlipped ? t("backLabel") : t("frontLabel")}
              </p>
            </div>
          </div>
          <div className="flex flex-col gap-3 rounded-2xl border bg-background/70 px-4 py-3 text-sm text-muted-foreground dark:border-white/10">
            <div className="flex items-center gap-2">
              <Layers className="h-4 w-4" />
              <span>
                {t("progressLabel")}: {progressCurrent} / {total}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <Clock className="h-4 w-4" />
              <span>
                {t("timerLabel")}: {formatTimer(elapsedSeconds)}
              </span>
            </div>
      ...
      >
        <div
          className={`study-card ${isFlipped ? "is-flipped" : ""} ${colorClass}`}
        >
          <div className="study-card-inner" onClick={handleFlip}>
        ...
        <div className="mt-3 flex items-center justify-between text-xs font-semibold text-muted-foreground">
          {colorTag ? <span className="uppercase">{colorTag}</span> : <span />}
      ...
      {state === "showBack" && (
        <div className="grid gap-3 md:grid-cols-3">
          ...
          <Button onClick={() => handleAnswer("easy")}>{t("easy")} (3)</Button>
        </div>
      )}
    </div>
  );
